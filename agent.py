...
CULTURE_EVENTS = [
    ...
    "קולנוע", "ערב תרבות", "מוזיקה חיה", "להקה", "זמר", "זמרת",
    "סרט", "הקרנה", "הקרנת בכורה", "מופע בכורה", "ערב הופעות",
    "מופע ילדים", "הצגת ילדים", "מחזמר", "בלט", "ריקוד",
    "גלריה", "פתיחת תערוכה", "ערב שירה", "ערב פיוט", "ג'אז",
    "מופע סטנדאפ", "ערב זמר", "מסיבה", "רייב", "דיג'יי",
    "אירוע תרבות", "מופע אמנות", "תיאטרון בובות", "קרקס",
    "פסטיבל סרטים", "פסטיבל מוזיקה", "פסטיבל אוכל", "פסטיבל רחוב",
    # צורות סמיכות שנפוצות בכותרות
    "תערוכת", "הופעת", "הצגת", "הרצאת", "מופע של", "ערב של",
    "פסטיבל ה", "הקרנת", "פתיחת תערוכת",
]
...
COMMUNITY_EVENTS = [
    ...
    "יריד אוכל", "שוק איכרים", "ערב הרצאות", "מפגש", "מושב",
    "סימפוזיון", "וובינר", "מיטאפ", "ערב עיון", "פאנל",
    "אירוע קהילתי", "אירוע חינם", "פעילות לילדים", "חוג",
    "מחנה קיץ", "קייטנה", "ערב משפחות", "פעילות נוער",
    # צורות סמיכות
    "סדנת", "כנס ה", "יריד ה", "מפגש", "קורס ה", "ערב עיון ב",
]
...
LOCAL_SITES = [
    ...
    # בתי חולים
    "rambam.org.il", "b-zion.org.il", "clalit.co.il",
    # רכבת ישראל
    "rail.co.il",
    # אתרי כרטיסים ואירועים (למשיכת אירועים)
    "eventer.co.il", "eventim.co.il", "ethos.co.il", "haifaff.co.il",
    "zappa-club.co.il", "leaan.co.il", "bravo.co.il", "haifa.events",
    "visit-haifa.org", "realhaifa.com", "radical.org.il", "ladaat.co",
    "muzi.co.il", "transwiki.co.il",
]
...
def build_queries():
    ...
    official_bodies = [
        ...
        # בתי משפט — סיקור מורחב
        "בית משפט השלום חיפה", "בית המשפט המחוזי חיפה",
        "בית משפט קריות", "בית דין חיפה", "פרקליטות מחוז חיפה",
        "גזר דין חיפה", "כתב אישום חיפה", "משפט חיפה",
        # מוסדות רשמיים נוספים
        "הטכניון", "אוניברסיטת חיפה", "רכבת ישראל חיפה",
    ]
    ...
    # אירועים מאתרי כרטיסים — חיפוש ממוקד
    event_sites = ["eventer.co.il", "eventim.co.il", "ethos.co.il", "haifaff.co.il", "haifa.events"]
    for city in ["חיפה", "הקריות", "קריון"]:
        for site in event_sites:
            queries.append(f"{quote(city)} site:{site} when:{GOOGLE_NEWS_DAYS_BACK}d")
